...
if  [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  sudo apt-get install silversearcher-ag python2 python3-pip fd-find fzf
...
if  [[ "$OSTYPE" == "darwin"* ]]; then
  brew install the_silver_searcher  prettier black stylua wget rigrep packer luarocks flake8 nodejs eslint python2 git any-key
  brew tap homebrew/cask-fonts
  brew install --cask font-hack-nerd-font
  python3 -m pip install --user --upgrade pynvim
        #Mac OSX
else
  sudo npm install --save-dev --save-exact prettier black stylua git any-key
